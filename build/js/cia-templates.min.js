...
/******/ 	return __webpack_require__(__webpack_require__.s = "./src/www/assets/js/cia-templates.js");
/******/ })
/************************************************************************/
/******/ ({

/***/ "./src/www/assets/js/cia-templates.js":
/*!********************************************!*\
  !*** ./src/www/assets/js/cia-templates.js ***!
  \********************************************/
/*! no exports provided */
...
/* harmony import */ var _pages_cia_templates_area_selection_cia__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../../pages/cia/templates/area-selection/cia */ "./src/www/pages/cia/templates/area-selection/cia.js");

window.addEventListener('DOMContentLoaded', Object(_pages_cia_templates_area_selection_cia__WEBPACK_IMPORTED_MODULE_0__["default"])());

/***/ }),

/***/ "./src/www/pages/cia/templates/area-selection/cia.js":
/*!***********************************************************!*\
  !*** ./src/www/pages/cia/templates/area-selection/cia.js ***!
  \***********************************************************/
...
var cia = function cia() {
  ...
  function localAreaSelect() {
    var pagePath = window.location.pathname; // Get the page's URL path
    var trimmedPathname = pagePath.replace(/\/$/, ''); // Remove the trailing slash from page's URL path
    var segments = trimmedPathname.split('/'); // Split the page's URL path
    var lastSegment = segments.pop(); // Gets the last part of the path
    var areaPages = ['coventry', 'dudley', 'sandwell', 'solihull', 'walsall', 'wolverhampton']; // Set area pages (where the area menu appears) '/' == Birmingham(default)
    var anyAreaPage = areaPages.includes(lastSegment); // Check areaPages to see if pagePath === any of them
    var splashUrl = '/cia/templates/area-selection'; // Splash URL,TODO: change this to umbraco URL
    var getSelectedArea = localStorage.getItem('selectedArea'); // Get selected area from localStorage
  ...
  function areaDropdownFooter() {
    var footerAreaSelect = document.querySelectorAll('footer .footer-dropdown > select');
    footerAreaSelect[0].addEventListener('change', function () {
      var selVal = footerAreaSelect[0].value;
      if (selVal === 'wolverhampton') {
        var modal = document.getElementById('wolvoModal');
  ...
  function videoLightbox() {
    var videoLightboxEle = document.querySelectorAll('.lightBoxVideoLink'); // Define lightbox links on page

    // Foreach lightbox element
    videoLightboxEle.forEach(function (element) {
      var getHref = element.getAttribute('href'); // Get the href attr
      ...
      if (getHref.indexOf('youtube.com/watch') !== -1) {
        element.setAttribute('href', "".concat(getHref.replace('watch?v=', 'embed/'), "?autoplay=1")); // Then set the href to the correct format
        // Else if the href contains the below string
      } else if (getHref.indexOf('youtu.be') !== 1) {
        element.setAttribute('href', "".concat(getHref.replace('youtu.be/', 'youtube.com/embed/'), "?autoplay=1") // Then set the href to correct format
        );
      }
    });
    var _window = window,
      SimpleLightbox = _window.SimpleLightbox; // Define what SimpleLightbox is so we can use it
  ...
  var fetchJustGiving = function fetchJustGiving() {
    fetch('https://apis.networkwestmidlands.com/justgiving/api', {
      ...
      var totalAmount = currencyFormat.format(data.totalRaised);
      // console.log(totalAmount);

      var replaceClass = document.querySelectorAll('.total-raised');
      if (replaceClass) {
        replaceClass.forEach(function (element) {
          var e = element;
  ...
  document.body.querySelectorAll('.shortanswer').forEach(function (node, i) {
  ...
  if (document.querySelector('.umbraco-forms-form') != null) {
    ...
    var findTotal = function findTotal() {
      var arr = document.querySelectorAll('.item-cost div input');
      var total = 0;
...
/* harmony default export */ __webpack_exports__["default"] = (cia);

/***/ })

/******/ });
//# sourceMappingURL=cia-templates.min.js.map